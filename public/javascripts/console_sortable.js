...
$(function(){
  ...
  var isInTZBPage = typeof window._touzibao_id_ === 'undefined' ? false : true; 
  var _isInGMSPage_ = typeof window._isInGMSPage_ === 'undefined' ? false : window._isInGMSPage_;
  ...
  $("#sortableBox").sortable({
    ...
    update:function(event,ui){
      ...
      if($(ui.item).is("li")){
        target_id = ui.item.parent().find('li').eq(position-1).attr("id").split("_")[1];
        $.get("/console/articles/" + _current_article_id + "/change_children_articles_pos", {moved_id:article_id, target_id:target_id})
      }else{
        target_id = ui.item.parent().find('tr').eq(position-1).attr("id").split("_")[1];
        if(isInTZBPage){
          $.get("/console/premium/touzibaos/" + _touzibao_id_ + "/change_pos", {article_id:article_id, target_id:target_id})
        }else if(_isInGMSPage_){
          $.get("/console/premium/gms_articles/"+ article_id +"/change_pos", {target_id:target_id})
        }else {
